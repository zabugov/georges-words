...
## Codex commercialization review for Claude (2026-07-06)

Zach asked Codex to independently review the commercialization plan while Claude was implementing parts of it. Overall verdict: the plan is directionally good and the architecture does not block commercialization. The biggest missing pieces are not core dictation features; they are the boring commercial-operational surfaces that make a paid product trustworthy and supportable. Suggested changes/actions for Claude to consider:

- [ ] **Make the current public download path actually work before sharing it.** The live Pages workflow currently fails until repo Settings → Pages is set to GitHub Actions. Also verify the latest GitHub Release has the stable `GeorgesWords.dmg` asset that `site/index.html` links to; the workflow now creates it, but any already-published latest release may not have it. Do not send nontechnical users to the page until a fresh install from that exact button has been tested.
- [ ] **Move EdDSA update signing earlier: required before taking money, not merely before “hundreds of users.”** A paid app with an unsigned Sparkle archive feed is an avoidable trust gap. Finish `SUPublicEDKey`, add the private-key secret, produce one signed appcast, and test an update from an older paid/distribution build before charging.
- [ ] **Add real customer-facing legal pages before payment: Privacy, Terms/EULA, Refund Policy, and Support.** Keep the repo without an open-source license, but the distributed app still needs user-facing terms that say what the buyer is allowed to do, refund rules, warranty limits, update expectations, and that the app is sold direct. The one-paragraph privacy statement is good marketing copy, not enough commercial plumbing by itself.
- [ ] **Add a third-party notices file/page.** About now credits the key components, but commercialization should also ship or link a `THIRD_PARTY_NOTICES` page/file covering Parakeet/CC-BY-4.0 attribution, Ollama, Qwen, WhisperKit, FluidAudio, Sparkle, and exact model/license versions. Treat this as a release artifact, not just an About string.
- [ ] **Decide the paid model before implementing license checks.** Recommended first paid version: direct one-time purchase with generous updates for a stated period, no accounts, no always-online DRM. If using Paddle/Lemon Squeezy, prefer their hosted checkout and license-key issuance over building billing. In-app activation should store a local signed entitlement and only contact the provider on explicit activation/deactivation/manual refresh; do not make normal dictation dependent on a network call.
- [ ] **Separate public support from private support.** GitHub Issues/Discussions are fine for public bugs and feature requests, but diagnostic reports can include app paths, bundle IDs, installed versions, and environment details. Add a support email or private intake form before asking normal customers to attach diagnostics, and update the bug template so users do not accidentally post private machine details publicly.
- [ ] **Redact paths in diagnostic reports.** `DiagnosticReport` currently includes the full app bundle path. For public issues and paid support, report install channel/location category instead (`/Applications`, source checkout, other) and redact the user/home path. Keep the “no transcripts/audio/dictionary/history” guarantee intact.
- [ ] **Promote privacy controls 8.1–8.3 into the pre-commercial checklist.** History retention, correction-learning off switch, and per-app exclusions are not only “family member” polish; they are directly tied to the privacy promise once strangers use the app for work, legal, medical, banking, or private messages.
- [ ] **Run a closed-beta support rehearsal before charging.** Minimum useful test: 10 nontechnical Macs if possible, fresh DMG install, first-run permissions, first model downloads, dictation into Mail/browser/chat/docs/terminal-like fields, update via Sparkle, diagnostic export, uninstall/factory reset, and one intentionally unsupported Mac. Track install failure rate and top support questions before deciding price or launch copy.
- [ ] **Add a security/contact policy.** Even without telemetry or cloud dictation, the app downloads code/model artifacts, runs a local engine, uses Accessibility, and self-updates. A short `SECURITY.md` or website section with a private report address is enough for launch.
- [ ] **Treat release notes as part of support, not only marketing.** 9.9 should include Sparkle release notes, website release notes, and a short “known issues” section for compatibility problems. This will reduce duplicate support reports.
- [ ] **Do not overbuild business infrastructure yet.** Avoid accounts, sync, admin dashboards, usage analytics, subscriptions, teams, or license servers beyond activation until there is real demand. The product’s strongest differentiator is local-first simplicity; commercialization should preserve that.
