...
2. **Test the checkout build** (`app/build.sh`), exercising the 2026-07-07 batch: correction learning (2.5 — fix a word within a minute, expect the Dictionary badge + pill notice), speculative polish (1.2 — pause before releasing; timing line should say "polish done during a pause"), command mode (4.4 — set a command key in Settings → Hotkeys first), style matching (3.3 — paste a sample under Settings → Your writing style), the Privacy section (8.1/8.2/8.3), microphone picker + silence warning (6.5), the Troubleshooting field tester (6.6), settings export/import (7.8), the menu-bar Use Unpolished Version / Undo Last Insertion (3.7/5.5), and — with a couple of dictionary terms — the experimental "Boost my dictionary words" toggle (2.2, expect a ~100 MB one-time download).
...
- [ ] **2.2 (S)** **Verify dictionary boosting on-device.** Wired 2026-07-07 behind a default-off toggle (Settings → Speech recognition → "Boost my dictionary words"): CTC word spotter + rescorer on the Parakeet final/speculative passes, fail-open, lazy ~100 MB model download on first use. Verify: enable with a few dictionary terms, dictate a mishearing-prone name, check debug.log for "Dictionary boost" lines and the latency feel. Later: learned corrections as aliases (needs the programmatic-vocabulary API rather than the file loader), and WhisperKit prompt tokens for the fallback (bug-check WhisperKit #372 first).
