...
**Pending owner actions (updated 2026-07-07):** all the manual clicks are now done. The one thing left is gated on cutting the next release:

- [ ] **Test the download-page button** — after the next release is cut, do one fresh install through the site's exact "Download" button (`GeorgesWords.dmg`); only then share the link (`https://zabugov.github.io/georges-words/`) with anyone non-technical. The button 404s until that release ships the stable-named asset — see 9.2. The Releases page links work today.

*(Done 2026-07-07: repo made public; GitHub Pages source set to "GitHub Actions" and the download page deployed live; Discussions enabled (9.3); private vulnerability reporting enabled; Sparkle EdDSA fully wired — `SUPublicEDKey` in Info.plist + `SPARKLE_ED_PRIVATE_KEY` repo secret both set, so the next release is the first signed one (7.10); `homebrew-georges-words` tap repo created — populating the cask is now a maintainer task, not an owner click (9.6).)*
...
- [x] **9.3** ~~Issue templates + Discussions~~ — templates shipped 2026-07-06 (bug report asks for the 9.5 diagnostic file); Discussions enabled 2026-07-07.
...
- [ ] **9.6 (S)** **Homebrew cask** — tap repo `zabugov/homebrew-georges-words` created 2026-07-07. Remaining (maintainer task, not an owner click): add the cask file to the tap and wire the release workflow to bump the cask's version + sha256 on each release (push to the tap via token). Best done right after the next release, since the cask needs a real stable `GeorgesWords.dmg` to checksum. Official homebrew/cask still requires notability we don't have — this self-tap is the interim path (`brew tap zabugov/georges-words && brew install --cask georges-words`).
- [ ] 7.10 (EdDSA feed signing) — **required before taking money** (review, adopted: an unsigned update feed on a paid app is an avoidable trust gap; verify one signed appcast + one real update before charging). FULLY WIRED 2026-07-07: pipeline signs the DMG and stamps `sparkle:edSignature` into the appcast; `SUPublicEDKey` in Info.plist + `SPARKLE_ED_PRIVATE_KEY` repo secret both in place — so the **next release is the first EdDSA-signed one**. The pipeline guards misconfiguration (`test -n "$ED_SIG"` fails the build if the secret is set but signing produces nothing). **Remaining verification (do at the next release):** confirm `sparkle:edSignature` actually landed in that release's appcast entry, and that one real end-to-end update succeeds, before relying on it. Note: builds already installed on family Macs (no `SUPublicEDKey`) update via the Developer-ID fallback and are unaffected; the signature requirement only binds copies installed from the next release onward.
