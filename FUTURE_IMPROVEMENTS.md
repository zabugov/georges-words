...
2. **Test the checkout build** (`app/build.sh`) by walking **[docs/qa-checklist.md](docs/qa-checklist.md)** top to bottom (~20 min). It's a ticked, step-by-step version of the whole 2026-07-07 batch — correction learning (2.5), speculative polish (1.2), command mode (4.4), the Privacy section (8.1/8.2/8.3), microphone picker + silence warning (6.5), the Troubleshooting field tester (6.6), settings export/import (7.8), and the menu-bar Undo AI Rewording / Undo Last Insertion (3.7/5.5). Each passing test retires its backlog item.
...
- [ ] **6.8 (S)** **App-owned speech-model cache root.** FluidAudio and WhisperKit cache models in directories shared by any app using those frameworks; factory reset therefore deletes model folders that another such app could in principle also be using (scoped today to Parakeet-named folders and the exact whisperkit-coreml repo — worst case another app re-downloads). Accepted trade-off for now (an explicit "Erase Everything" exists to reclaim the gigabytes, and no other FluidAudio app is realistically present on family Macs); revisit if either framework grows a supported per-app cache-directory override.
...
- [ ] **2.2 (M)** **Dictionary boosting — REMOVED 2026-07-22 (owner decision: simplify before sharing).** Fully deleted from the app the same day it was parked — the "Boost my dictionary words" toggle, the `dictionaryBoostEnabled` setting, and the CTC word-spotter rescore in `Transcriber` (restore all three from git history if re-attempted; the phonetic dictionary matching in `TranscriptCleaner` covers the everyday need meanwhile). On-device history in one day: over-fired at floor 0.52 (fixed: 0.70 + hard replacement cap) → then correctly fixed "Abugov" at the source on natural speech → then swapped an *unknown* proper name ("Marina Cremonese"-class) for random dictionary terms, including an email. Vocab hygiene shipped (name-shaped terms only), but OOV proper names remain the blind spot: weak decoder evidence is exactly where a boosted term wins wrongly. Any re-attempt needs threshold tuning against debug.log data on real audio, and must pass the QA §11 gate: an unknown proper name dictated three times must never come out as a dictionary word or email. Research: docs/research/dictionary-biasing-in-asr.md. (Note: the same symptom can come from Full-mode LLM polish — emails are excluded from the model's DICTIONARY line too.)
- [ ] **3.2 (S)** **Per-app style notes — REMOVED 2026-07-22, revisit later (owner decision: advanced feature).** Was: a Settings section attaching a free-text note to an app ("use markdown headings", "keep it terse"), matched by bundle ID and appended to the STYLE line in full-rewrite polish. Shipped 2026-07-07, removed before any release, so no user ever had it. To bring it back: restore from git history (`AppInstruction` struct, `appInstructions` in AppSettings/SettingsBackup, the Settings section, the `customInstruction` parameter through LLMFormatter/AppDelegate/SpeculativeGuess, and AppInstructionTests). Any notes typed during testing still sit in UserDefaults under `AppInstructions` — the re-add finds them intact.
- [ ] **3.3 (S)** **Personal style matching — REMOVED 2026-07-22, revisit later (owner decision: advanced feature).** Was (ADR 0011, design still valid): paste one writing sample per tone profile under Settings → Your writing style; full polish gained a VOICE line imitating it, and the sample rode the speculative-polish cache key. Shipped 2026-07-07, removed before any release. To bring it back: restore from git history (`styleSamples`/`styleSample(for:)` in AppSettings/SettingsBackup, the Settings section, the `styleSample` parameter through LLMFormatter/AppDelegate/SpeculativeGuess) and then do the on-device verification that was never finished: paste real samples, dictate with Full polish, judge the imitation. Samples typed during testing still sit in UserDefaults under `StyleSamples`.
- [ ] **3.5 (S)** **Grow the few-shot bank.** Keep a small local corpus of messy transcript → ideal output examples from real failures.
- [ ] **3.7 (S)** **Verify “Undo AI Rewording” on-device.** Landed 2026-07-07 (renamed 2026-07-22 — "unpolished" overpromised: the recovered text is pre-AI but still rule-tidied, so fillers stay stripped). Swaps the pre-AI text back in place; clipboard fallback otherwise. If a true raw-ASR variant is ever wanted, that's a separate small feature.
- [ ] **4.1 (M)** **Multilingual dictation.** Add language auto-detect and multilingual model support, including mid-sentence language changes if practical. Note (2026-07-22): Parakeet v3 already code-switches SPONTANEOUSLY — an English "dot" once transcribed as Cyrillic "дот" and broke email assembly (a transliteration table in SpokenContacts patches the known cases). Part of this work is deciding whether the decoder should be pinned to English until multilingual is a real feature — check whether FluidAudio's AsrManager config exposes a language hint.
