...
2. **Test the checkout build** (`app/build.sh`) by walking **[docs/qa-checklist.md](docs/qa-checklist.md)** top to bottom (~20 min). It's a ticked, step-by-step version of the whole 2026-07-07 batch — correction learning (2.5), speculative polish (1.2), command mode (4.4), the Privacy section (8.1/8.2/8.3), microphone picker + silence warning (6.5), the Troubleshooting field tester (6.6), settings export/import (7.8), the menu-bar Use Unpolished Version / Undo Last Insertion (3.7/5.5), and the experimental "Boost my dictionary words" toggle (2.2). Each passing test retires its backlog item.
...
- [ ] **2.2 (M)** **Dictionary boosting — PARKED (owner decision, 2026-07-22).** On-device history in one day: over-fired at floor 0.52 (fixed: 0.70 + hard replacement cap) → then correctly fixed "Abugov" at the source on natural speech → then swapped an *unknown* proper name ("Marina Cremonese"-class) for random dictionary terms, including an email. Vocab hygiene shipped (name-shaped terms only), but OOV proper names remain the blind spot: weak decoder evidence is exactly where a boosted term wins wrongly. Keep the toggle OFF (it is off by default) until someone can tune thresholds against debug.log data on real audio. The "unknown proper name must survive untouched" sentence joins §11 of the QA checklist as the gate for any re-attempt. (Note: the same symptom can come from Full-mode LLM polish — emails are now excluded from the model's DICTIONARY line too.)
