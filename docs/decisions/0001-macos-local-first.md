...
We are building a Wispr Flow–style dictation app. The owner's machine is a MacBook, and the non-negotiable requirement is that **no audio leaves the device** — Wispr Flow itself transcribes in the cloud, which is exactly what we want to avoid.
...
- Some Wispr Flow features that depend on server-side LLMs (heavy tone rewriting, cross-device sync) will be reimplemented with small local models or rules, or dropped.
